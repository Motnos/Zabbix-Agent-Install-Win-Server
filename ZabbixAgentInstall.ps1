...
$zabbixInstallFiles = '\\networkshare\zabbix_agent_files' #Location of an existing install (copy it to a UNC share)
$zabbixConfFile = $zabbixInstallPath+'zabbix_agentd.conf'
#$zabbixAgentVersion = '4.4.10.2400'  - Not used yet

$service = Get-Service -Name 'Zabbix Agent' -ErrorAction SilentlyContinue
if ($service -ne $null)  #Checks if Zabbix service already installed, if it is - exit script.
{
...
if ($firewallRule -ne $null)  #Checks if Zabbix firewall exception exists
{
...
$zabbixConfPath = 'Include='+$zabbixConfFile
